...
class iLQRMatrices(object):
    def __init__(self, dynamics, cost_function):
        ...
        self.get_fx_batched = jit(vmap(dynamics.get_A_matrix))
        self.get_fu_batched = jit(vmap(dynamics.get_B_matrix))
    ...
    def get_cost(self, states, actions):
        l = self.get_l_batched(states[:-1], actions).sum()
        ln = self.get_l_terminal(states[-1, None]).sum()
        return l + ln

    def get_matrices(self, states, actions):

        fx = self.get_fx_batched(states[:-1], actions)
        fu = self.get_fu_batched(states[:-1], actions)
        lx = self.get_lx_batched(states[:-1], actions)[..., None]
        lxx = self.get_lxx_batched(states[:-1], actions)
        lu = self.get_lu_batched(states[:-1], actions)[..., None]
        luu = self.get_luu_batched(states[:-1], actions)
        lux = self.get_lux_batched(states[:-1], actions)

        lx_N = self.get_lx_terminal(states[-1, None])[..., None]
        lxx_N = self.get_lxx_terminal(states[-1, None])
...
class iLQR(object):
    ...
    def backward_pass(self, x, u):
        fx, fu, lx, lxx, lu, luu, lux = self.ilqr_matrices.get_matrices(x, u)
    ...
    def forward_pass(self, x_0, x_bar, u_bar, K, k, alpha):
    ...
    def solve(
        ...
        for i in range(self.config.iters):
            ...
            for alpha in alphas:
                ...
                x_new, u_new = self.forward_pass(x_0, x, u, K, k, alpha)
                J_new = self.ilqr_matrices.get_cost(x_new, u_new)
                if J_new < J:
                    x, u = x_new, u_new
...
if __name__ == "__main__":
    ...
    goal = jnp.array([0.0, 0.0, np.pi / 2.0, 0.0, 0.0, 0.0])
